			...
			if( $dni != "" && strlen( $dni ) == 8 )
			{
				...
				if($response)
				{
					...
					if( isset($result[4]) )
					{
						...
						$nombres = explode(' ', $rtn["nombres"]);
						$apellidos = explode(' ', $rtn["apellidos"]);

						$formateo = [
							"numero_de_documento" => $rtn["dni"],
							"primer_nombre" => $nombres[0],
							"segundo_nombre" => isset($nombres[1]) ? $nombres[1] : "",
							"ape_paterno" => $apellidos[0],
							"ape_materno" => isset($apellidos[1]) ? $apellidos[1] : "",
							"nombres" => $rtn["nombres"],
							"gvotacion" => $rtn["gvotacion"],
							"distrito" => $rtn["distrito"],
							"provincia" => $rtn["provincia"],
							"departamento" => $rtn["departamento"]
						];
